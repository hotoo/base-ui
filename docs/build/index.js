...
pages.forEach(function (name) {
  ...
  var page = fs.readFileSync(path.join(__dirname, templatePath, pagesPath, name), 'utf-8')
    ...
    , includes = fs.readdirSync(path.join(__dirname, templatePath, compPath))
  ...
  includes && includes.length &&
	  includes.forEach(function(name) {
            ...
            try {
                var ipage = fs.readFileSync(path.join(__dirname, templatePath, compPath, name), 'utf-8')
            } catch(e) {
            ...
            fs.writeFileSync(path.join(__dirname, '/../components', name.replace(/\.mustache$/, '') + '.html'), ipage, 'utf-8')	
	  })
